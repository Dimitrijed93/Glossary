<template>
  <div v-if="entries.list.length">
      <entry v-for="(entry, idx) in entries.list"  :key="idx" v-bind:data="entry" ></entry>
  </div>
</template>
...
export default {
  ...
  methods: {
    load(id) {
      axios.get(`${this.entries.BASE_URL}/entries/by-folder/${id}`)
        .then(response => {
            debugger
            this.entries["list"] = response.data;
        });
    },
    itemValue(folder) {
      return folder["name"];
    ...
    populate(response) {
      if (response) {
        ...
        this.folders["source"] = {
          id: response.data.sourceLanguage.id,
          text: response.data.sourceLanguage.name,
        };
        this.folders["destination"] = {
          id: response.data.destinationLanguage.id,
          text: response.data.destinationLanguage.name,
        };
      } else {
    ...
    deleteItem(item) {
      debugger;
      axios
        .delete(`${this.folders.BASE_URL}/folders/${this.folders["folderId"]}`)
        .then(() => {
          this.$emit("getAll");
          this.$bvModal.hide("type-modal");
        });
